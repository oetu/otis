...
def split_and_dump(params, normalise=True):
    ...
    for file in os.listdir(fetch_folder):
        if sub in file:
            ...
            try:
                ...
                raw.resample(200, n_jobs=8)

                ch_name = raw.ch_names
                raw_data = raw.get_data(units='uV')

                if normalise:
                    raw_data = ( raw_data - raw_data.mean(axis=-1, keepdims=True) ) / ( raw_data.std(axis=-1, keepdims=True) + 1e-9 )
...
if __name__ == "__main__":
    ...
    root = "/home/oturgut/data/TUAB/v3.0.1/edf/"
