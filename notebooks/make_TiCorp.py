...
# other_train_path = f"/vol/aimspace/users/tuo/data/processed/TiCorp/train_wo_mimic.pt"
# other_train = torch.load(other_train_path)

other_val_path = f"/vol/aimspace/users/tuo/data/processed/TiCorp/val_wo_mimic.pt"
other_vaĺ = torch.load(other_val_path)
...
if __name__ == '__main__':
    ...
    # ecgs_mimic_train = [("ecg", torch.from_numpy(sample)) for sample in ecgs_mimic[:int(0.9875*upper_bnd)]]
    # ticorp_train = ecgs_mimic_train + other_train

    ecgs_mimic_val = [("ecg", torch.from_numpy(sample)) for sample in ecgs_mimic[int(0.9875*upper_bnd):]]
    ticorp_val = ecgs_mimic_val + other_vaĺ

    # Save data
    output_path = f"/vol/aimspace/users/tuo/data/processed/TiCorp"
    print(f"SAVE DATA ({output_path})")
    # torch.save(ticorp_train, f"{output_path}/train_all.pt", pickle_protocol=4)
    torch.save(ticorp_val, f"{output_path}/val_all.pt", pickle_protocol=4)
