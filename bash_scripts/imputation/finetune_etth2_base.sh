            ...
            do
                ...
                do
                    ...
                    do
                        ...
                        subfolder="cos_weight$cos_weight/ncc_weight$ncc_weight/seed$sd/$model_size/t$time_steps/p$patch_height"x"$patch_width/wd$wd/m$mr"
                        ...
                        if [ "$path" = "tower" ]; then
                            cmd="python3 main_finetune_gen.py --output_projection $output_projection --downstream_task $downstream_task --seed $sd --patience $patience --crop_lower_bnd $crop_lower_bnd --crop_upper_bnd $crop_upper_bnd --max_delta $max_delta --jitter_sigma $jitter_sigma --rescaling_sigma $rescaling_sigma --ft_surr_phase_noise $ft_surr_phase_noise --input_channels $input_channels --input_electrodes $input_electrodes --time_steps $time_steps --patch_height $patch_height --patch_width $patch_width --ncc_weight $ncc_weight --cos_weight $cos_weight --model $model --batch_size $bs --epochs $epochs --accum_iter $acc_it --mask_ratio $mr --weight_decay $wd --blr $blr --warmup_epochs $warmup_epochs --data_path $data_path --val_data_path $val_data_path --num_workers $num_workers"
