...
#SBATCH --output=/vol/aimspace/users/tuo/otis/slurm_output/data/mimic-%A.out   # Standard output of the script (Can be absolute or relative path). %A adds the job id to the file name so you can launch the same script multiple times and get different logging files
#SBATCH --error=/vol/aimspace/users/tuo/otis/slurm_output/data/mimic-%A.err    # Standard error of the script
...
# initial_cmd="ulimit -n 4096"
# check_cmd="ulimit -n"
# echo $initial_cmd && $initial_cmd && $check_cmd
cmd="python3 ./notebooks/make_MIMIC.py"
echo $cmd && $cmd